package com.cn.xxx.commons.redis.serialize;
...
public class ListTranscoder <M extends Serializable> extends SerializeTranscoder{
	...
	public List<M> deserialize(byte[] in) {
		...
		try {
			...
			// String.format("Caught IOException decoding %d bytes of data", in
			// == null ? 0 : in.length) + e);
		} catch (ClassNotFoundException e) {
			// LoggerUtils.error(logger,
			// String.format("Caught CNFE decoding %d bytes of data",in == null
			// ? 0 : in.length) + e);
		} finally {
			close(is);
			close(bis);
